...
public class ClientUserManagement {
	...
	public static List<User> getAllUsers() {
		return Arrays.asList(Context.doGetRequest("users/getall",User[].class));	
	}

	/* adds a new user */
	public static User addUser(User user) {
		return  Context.doPostRequest("users/add",user,User.class);	
		 
	}

	/* tries to login and return a LoginResult which also has a token */
	public static LoginResult login(LoginInfo loginInfo) {
		return  Context.doPostRequest("users/login",loginInfo,LoginResult.class);
	}
